...
namespace MongoDBGenericRepositoryExampleProject.Core.Repository.MongoDB.Concrete;
...
public abstract class MongoDBRepositoryBase<T> : IDocumentDbRepository<T> where T : DocumentDbEntity
{
    ...
    {
        var options = new BulkWriteOptions { IsOrdered = false, BypassDocumentValidation = false }; // data eklemede şema kuralı varsa bu kurala uygun mu diye denetleme yapacak. 
